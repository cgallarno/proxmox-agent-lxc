...
`systemd/agent-gateway.service`, `systemd/agent-gateway@.service` (Hermes named profiles), `systemd/hermes-gateway-control.service` (Hermes sidecar), and `agent-config-watch.service` contain `@OC_USER@`, `@OC_HOME@`, `@AGENT_CMD@`, `@AGENT_PATH@` placeholders. `setup.sh` renders them via `sed` into `/etc/systemd/system/`. The `.path` watcher is generated dynamically from `$TRACKED_FILES`. Never edit the rendered copies — edit the templates in `systemd/`.
...
- **`hermes-gateway-control`** — Hermes-only client/sidecar. If `ENABLE_HERMES_GATEWAY_CONTROL=true`, `hermes-gateway-control.service` runs as root and exposes a restricted Unix socket owned by `$OC_USER`; non-root Hermes can request only `status`/`restart` for `agent-gateway.service` and `agent-gateway@<profile>.service`. This works from gateway sessions even when the gateway unit has `NoNewPrivileges=yes`.
